...
export default function ForgotPassword() {
  ...
  const sendOtp = async () => {
    if (!phone) {
      ...
      try {
        const res = await axios.post("http://localhost:7000/user/otp?type=forgot", { phone });
        if (res.status === 200) {
          setOpenPhone(false);
          setOpenOtp(true);
        }
  ...
  const register = async (e) => {
    ...
    if (!password || !confirmPassword) {
      ...
      const res = await axios.put("http://localhost:7000/user/forgotPassword", { phone, password });
      if (res.status === 200) {
        navigate("/login");
      }
    }
